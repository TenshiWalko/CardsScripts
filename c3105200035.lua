...
function s.initial_effect(c)
	--Shuffle 1 "Elemental HERO" monster and Special Summon 1 with a different name
...
function s.spfilter(c,e,tp,code)
	return c:IsSetCard(0x3008) and not c:IsCode(code)
...
function s.tdfilter2(c)
	return c:IsSetCard(0x3008) and c:IsMonster() and c:IsAbleToDeck()
end

function s.resfilter(c,sg)
	return c:IsSetCard(0x3008) and sg:IsExists(Card.IsSetCard,1,c,0x3008)
end
